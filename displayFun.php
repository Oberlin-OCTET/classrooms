...
<?php
function display($roomnumber, $roombuilding, $buildingcode) {
	 $sql_str_contact = "SELECT RoomNotes, RoomEquipment FROM rooms WHERE RoomNumber='$roomnumber' AND BuildingCode='$buildingcode' ";
	 $ContactInfo = mysql_query($sql_str_contact);
	 $contactNotes = mysql_fetch_array($ContactInfo);
	 
	 $ans = "<b>Schedule for <a href=Index.php?Step=1&building=$buildingcode&room=%&searched=1&cap=Capacity>".$roombuilding."</a> ".$roomnumber."</b>&nbsp;-&nbsp;".$contactNotes['RoomEquipment']."<br>";
	 $ansContact= $contactNotes['RoomNotes']."<blockquote>";
	 $sql_str = "SELECT * FROM records WHERE room='$roomnumber' AND building='$roombuilding' ORDER BY times";
	...
	while($row = mysql_fetch_array($result)) {
		...
		for($i=0; $i<7; $i++) {
			if($days[$i+1]) array_push($week[$i], $row[times]."      ".$row[title]." - <i><a href=Index.php?Step=6&first=$row[instructor] title='schedule for $row[instructor]'>".$row[instructor]."</a></i><br>"); 
		}
	}
	...
	return $ans.$ansContact.$ans1.$ans2.$ans3.$ans4.$ans5.$ans6.$ans7."</blockquote>";
